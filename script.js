...
function displayFeaturedZones(list){ 
  ...
  if(!list.length){ featuredContainer.innerText='No featured zones.'; return; }
  list.forEach(f=>{
    ...
    btn.textContent=f.name; btn.onclick=(e)=>{e.stopPropagation(); openZone(f)}; 
    d.appendChild(btn); featuredContainer.appendChild(d); 
  }); 
  lazyLoadImages();
}
...
function displayZones(list){ 
  container.innerHTML=''; 
  if(!list.length){container.innerText='No zones found.'; return;} 
  list.forEach(file=>{
    const d=document.createElement('div');d.className='zone-item'; d.onclick=()=>openZone(file); 
    const img=document.createElement('img'); 
    img.dataset.src=file.cover.replace('{COVER_URL}', coverURL); 
    img.loading='lazy'; d.appendChild(img); 
    const btn=document.createElement('button'); btn.textContent=file.name; 
    btn.onclick=(e)=>{e.stopPropagation(); openZone(file)}; 
    d.appendChild(btn); container.appendChild(d);
  }); 
  lazyLoadImages(); 
...
function openZone(file){
  ...
  fetch(url+'?t='+Date.now()).then(r=>r.text()).then(html=>{
    ...
    zoneFrame.contentDocument.open(); zoneFrame.contentDocument.write(html); zoneFrame.contentDocument.close();
...
function aboutBlank(){ 
  ...
  if(!zone) return; 
  fetch(zone.url.replace('{HTML_URL}', htmlURL)+'?t='+Date.now()).then(r=>r.text()).then(t=>{ 
...
function populateFilters(json){ 
  ...
  while(opt.children.length>1) opt.removeChild(opt.lastElementChild); 
  alltags.forEach(tag=>{
...
function tabCloak(){ closePopup(); document.getElementById('popupTitle').textContent='Tab Cloak'; document.getElementById('popupBody').innerHTML = `<label>Set tab title:</label><br><input oninput="document.title=this.value" placeholder="Enter new tab name"><br><br><label>Set tab icon URL:</label><br><input oninput="(function(u){const l=document.querySelector('link[rel~=icon]')||document.createElement('link'); l.rel='icon'; l.href=u; document.head.appendChild(l);})(this.value)" placeholder="Icon URL">`; document.getElementById('popupOverlay').style.display='flex'; }
